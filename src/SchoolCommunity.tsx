...
function Toast({ msg, onClose }: { msg:string; onClose:()=>void }) {
  ...
  return <div style={{ position:"fixed", bottom:24, left:"50%", transform:"translateX(-50%)", background:C.green, color:"#000", borderRadius:10, padding:"10px 20px", fontWeight:700, fontSize:14, zIndex:999 }}>{msg}</div>;
}
...
function AdminIPLog() {
  ...
  return (
      <div>
        ...
        {loading ? <Spinner /> : entries.length === 0 ? <p style={{ color:C.muted, fontSize:12 }}>No data</p> : (
...
function AuthPage({ auth, onLogin, onLogout }: { auth:AuthState; onLogin:(d:{token:string;userId:string;email:string})=>void; onLogout:()=>void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "register") {
        ...
        tokenStorage.setAccess(data.accessToken); tokenStorage.setRefresh(data.refreshToken);
        onLogin({ token:data.accessToken, userId:data.user.id, email:data.user.email });
      }
    } catch(e) { setError((e as Error).message); }
    setLoading(false);
  };
  const handleLogout = async () => { try { await authAPI.logout(); } catch { /**/ } tokenStorage.clear(); setAccessToken(null); onLogout(); };

  if (auth.userId) return (
      <div style={css.page}><div style={{ ...css.card, maxWidth:440, margin:"0 auto", textAlign:"center" }}>
        ...
        {auth.profile && <p style={{ color:C.dim, fontSize:13, marginBottom:8 }}>Grade {auth.profile.grade} Class {auth.profile.class}{auth.profile.is_admin && <span style={{ ...css.badge(C.red), marginLeft:8 }}>Admin</span>}</p>}
        <span style={{ ...css.badge(C.green), display:"inline-block", marginBottom:20 }}>Session Active</span>
        {auth.profile?.is_admin && <div style={{ ...css.card, background:"#0a1220", textAlign:"left", marginBottom:16 }}><h3 style={{ fontSize:14, fontWeight:700, color:C.red, marginBottom:10 }}>Admin - IP Log</h3><AdminIPLog /></div>}
  ...
  return (
      <div style={css.page}><div style={{ maxWidth:460, margin:"0 auto" }}>
        ...
        <div style={css.card}>
          ...
          {mode === "register" && <>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
              <Field label="Grade"><select style={css.input} value={form.grade} onChange={upd("grade")}>{["1","2","3"].map(g => <option key={g} value={g}>Grade {g}</option>)}</select></Field>
              <Field label="Class"><select style={css.input} value={form.classNum} onChange={upd("classNum")}>{["1","2","3","4","5","6"].map(c => <option key={c} value={c}>Class {c}</option>)}</select></Field>
            </div>
          </>}
...
function BambooPage({ auth }: { auth:AuthState }) {
  ...
  const handleLike = async (p:Post) => { if (!auth.userId) return; try { await postsAPI.like(p.id); setPosts(prev => prev.map(x => x.id===p.id ? {...x, likes:x.likes+1} : x)); } catch { /**/ } };
  return (
      <div style={css.page}>
        ...
        <textarea style={{ ...css.input, minHeight:88, resize:"vertical", marginBottom:10 }}
                  ...
                  value={content} disabled={!auth.userId||submitting} onChange={e => setContent(e.target.value)} />
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <span style={{ fontSize:12, color:C.muted }}>Anonymous - IP logged</span>
            <button style={css.btn(C.green)} onClick={submit} disabled={!auth.userId||submitting}>{submitting ? "Posting..." : "Post"}</button>
          </div>
          {error && <div style={css.errorBox}>{error}</div>}
        </div>
        {loading ? <Spinner /> : posts.map(p => (
            <div key={p.id} style={css.postCard(C.green)}>
              ...
              <div style={{ display:"flex", gap:14, fontSize:13, color:C.muted, alignItems:"center" }}>
                <button style={{ background:"none", border:"none", color:C.muted, cursor:"pointer", fontSize:13, padding:0 }} onClick={() => handleLike(p)}>like {formatCount(p.likes)}</button>
...
function StudyPage({ auth }: { auth:AuthState }) {
  ...
  const submit = async () => {
    ...
    setSub(true);
    try { await postsAPI.create({ board:"study", content:form.content, title:form.title, subject:form.subject }); setForm({ title:"", content:"", subject:"Math" }); await load(1); setPage(1); } catch { /**/ }
    setSub(false);
  };
      ...
      <div style={css.page}>
        ...
        {auth.userId && (
            <div style={{ ...css.card, marginBottom:"1.5rem", borderColor:C.green+"40" }}>
              ...
              <Field label="Subject"><select style={css.input} value={form.subject} onChange={e => setForm(p => ({...p, subject:e.target.value}))}>{SUBJECTS.slice(1).map(s => <option key={s}>{s}</option>)}</select></Field>
              <Field label="Content"><textarea style={{ ...css.input, minHeight:70, resize:"vertical" }} placeholder="Content or link" value={form.content} onChange={e => setForm(p => ({...p, content:e.target.value}))} /></Field>
...
function ChatPage({ auth }: { auth:AuthState }) {
  ...
  useEffect(() => {
    setLoading(true); setMsgs([]); joinRoom(room);
    chatAPI.messages(room).then(data => setMsgs(data)).catch(() => {}).finally(() => setLoading(false));
    const off = onReceiveMessage(msg => { setMsgs(prev => prev.some(m => m.id===msg.id) ? prev : [...prev, msg]); });
    return () => { off(); leaveRoom(room); };
  }, [room]);
  useEffect(() => { endRef.current?.scrollIntoView({ behavior:"smooth" }); }, [msgs]);
  const send = () => {
    const text = input.trim(); if (!text||!auth.userId) return;
    setInput("");
  ...
  return (
      <div style={{ ...css.page, display:"flex", flexDirection:"column", height:"calc(100vh - 58px)", paddingBottom:0 }}>
        ...
        <div style={{ display:"flex", gap:8, flexShrink:0, paddingBottom:"1rem" }}>
          <input style={{ ...css.input, flex:1 }} placeholder={auth.userId ? "Type message... (Enter)" : "Login to chat"}
                 disabled={!auth.userId} value={input} onChange={e => setInput(e.target.value)}
...
function CommunityPage({ auth }: { auth:AuthState }) {
  ...
  const submitBlog = async () => {
    if (!form.title.trim()||!form.content.trim()) return;
    setSub(true);
    try { await postsAPI.create({ board:"blog", content:form.content, title:form.title }); setForm({ title:"", content:"" }); await loadBlog(); setToast("Post published!"); } catch { /**/ }
    setSub(false);
  };
  const submitShort = async () => {
    ...
    setSub(true); setProg(`Uploading... (${formatFileSize(videoFile.size)})`);
    try { await storageAPI.uploadShort(videoFile, videoTitle); setVF(null); setVT(""); await loadShorts(); setToast("Short uploaded!"); }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    authAPI.me().then(profile => setAuth({ userId:profile.id, email:null, profile, token }))
